...
void FrogMachineTransformation::on_pB_convert_clicked()
{
	...
	//新建一个线程进行文档转换
	QConvert_thread* convert_thread = new QConvert_thread(sourceFile, finalFile, mmach, offset, G_workpiece);
	//connect(convert_thread, &QThread::finished, convert_thread, &QObject::deleteLater);
	connect(convert_thread, SIGNAL(ConvertFinish(int)), this, SLOT(ReceiveFinishSig(int)));
	convert_thread->start();
}


//收到文件转换完成信号
void FrogMachineTransformation::ReceiveFinishSig(int et) {
	QString dlgTitle;
	QString strInfo;
	switch (et)
	{
